...
import {
  fiberstarAllowedCities,
  fiberstarHomepassApiEmail,
...
import {
  ...
  fetchFiberstarConfig,
  fetchFiberstarHomepass,
  saveFiberstarConfig,
  saveFiberstarHomepass,
  updateFiberstarHomepass,
} from './database'
      ...
      if (result != undefined) {
        ...
        if (authResponse.time_request != undefined) {
          ...
          const diff = Math.floor(
            (timeNow.getTime() - timeRequest.getTime()) / 1000,
          )
...
async function login(): Promise<any> {
  try {
    const token = Buffer.from(
      `${fiberstarHomepassApiEmail}:${fiberstarHomepassApiPassword}`,
    ).toString('base64')
    const response = await getAxiosInstance().post(
      FiberstarEndpoint.API_AUTH_TOKEN,
      {},
      {
        headers: {
...
async function getFiberstarHomepassType(): Promise<any> {
  try {
    const homepassTypes = await getAxiosInstance().get(
      FiberstarEndpoint.API_TIPE_HOMEPASS,
    )
    await saveFiberstarConfig(
      FiberstarConfigKeys.HOMEPASS_TYPES,
      homepassTypes.data.message,
    )
...
async function getFiberstarHomepassDetails(body: any, page: number) {
  try {
    const response = await getAxiosInstance().post(
      FiberstarEndpoint.API_HOMEPASS_DETAILS,
      body,
      {
        params: {
          page: page,
    ...
    response.data.message.data.forEach(async (data: any) => {
      ...
      const homepassId = await fetchFiberstarHomepass(
        'fiberstar',
        data.homepass_id,
        'homepass',
      )
      if (homepassId != undefined && homepassId > 0) {
        await updateFiberstarHomepass(
          homepassId,
          data.homepassed_coordinate,
          data.homepass_id,
          address,
          'homepass',
          rfsDate,
      ...
      const splitterId = await fetchFiberstarHomepass(
        'fiberstar',
        data.splitter_id,
        'splitter',
      )
      if (splitterId != undefined && splitterId > 0) {
        await updateFiberstarHomepass(
          splitterId,
          data.spliter_distribusi_koordinat,
          data.splitter_id,
          address,
          'splitter',
          rfsDate,
...
async function getFiberstarUpdatedHomepass(body: any, page: number) {
  try {
    const response = await getAxiosInstance().post(
      FiberstarEndpoint.API_UPDATED_HOMEPASS,
      body,
      {
        params: {
          page: page,
...
async function getFiberstarDeletedHomepass(body: any, page: number) {
  try {
    const response = await getAxiosInstance().post(
      FiberstarEndpoint.API_DELETED_HOMEPASS,
      body,
      {
        params: {
          page: page,
